...
- [ ] **Backup offsite S3** — **reporté (budget, 2026-05-17)**. Plan `docs/plans/public-launch-hardening-2026-05-17.md` Task 1 ; local Coolify OK. Copie hors VPS + restore test offsite à faire quand budget ou solution gratuite/manual documentée ; pas bloquant pour beta limitée actuelle.
...
- [x] **Visibilité quota GitHub (L1 + L2)** — livré 2026-05-17. Logs structurés + `GET /api/admin/github/quota` + `ingestion` sur `/api/status/public` ; runbook `docs/ops-mcp-coolify-hardening.md`.
- [ ] **Email produit réel (watch/digest)** — Task 5 ; runbook `docs/ops-email-notification-runbook.md` ; test canal OK, livraison inbox produit à prouver.
...
- [x] **Awesome corpus import (tooling)** — collector, allowlist, import script, Task 4 short-circuit livrés 2026-05-17. Voir `docs/corpus/awesome-import.md`. **Import prod** : après deploy short-circuit, revue `awesome-candidates-approved.json`, puis batches.
