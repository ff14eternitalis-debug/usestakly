...
- L'app active est organisée autour de `discover`, `repo-detail`, `watchlist`, `notifications`, `login`.
- État global : Zustand pour l'auth / locale, React Query pour la data serveur.
- Réseau : fetch direct via `src/lib/api-client.ts` (`credentials: "include"`, base `VITE_API_BASE_URL`).
